...
SELECT 
	MIN(first_contact_date) as earliest_date,
	MAX(first_contact_date) as latest_content
FROM marketing.marketing_qualified_leads;
...
SELECT 
	SUM(CASE WHEN mql_id IS NULL THEN 1 ELSE 0 END) AS mql_id_null,
	SUM(CASE WHEN won_date IS NULL THEN 1 ELSE 0 END) AS won_date_null
FROM marketing.marketing_closed_deals;
...
SELECT * 
FROM marketing.marketing_closed_deals as c
JOIN marketing.marketing_qualified_leads as m
...
/* Interpretation: NULL values indicate missing or unreported attributes 
